...
void mergeClusters (Dendrogram *dendA, double **dist, int n, int method) {
    ...
    Dendrogram cluster = calloc(1,sizeof(Dendrogram));
    cluster->left->left = dendA[x]->left;
    cluster->left->right = dendA[y]->left;
    for (i = 0; i < n; i++) {
        if (i > x && i < y) dendA[i-1] = dendA[i];
        if (i > y) dendA[i-2] = dendA[i]; 
    }
    // reassign the new cluster to the end
    dendA[n-1] = cluster;
    printf("%d, %d\n", x,y);
...
Dendrogram LanceWilliamsHAC(Graph g, int method) {
    ...
    for (int i = 0; i < n; i++) {
        dendA[i] = calloc(1,sizeof(Dendrogram));
        dendA[i]->left->vertex = i;
        dendA[i]->left->left = dendA[i]->left->right = NULL;
        dendA[i]->right = NULL;
    }
